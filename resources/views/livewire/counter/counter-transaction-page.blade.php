            ...
            <div class="mb-6">
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    @forelse ($counter->services as $service)
                        <span
                            class="inline-flex items-center px-3 py-1 rounded-full bg-gray-200 text-gray-700 text-xs font-medium">
                            {{ $service->name }}
                        </span>
                    @empty
                ...
                <div class="flex flex-col md:flex-row md:items-center md:gap-4 bg-gray-50 px-4 py-3 rounded-lg">
                    <div class="flex items-center gap-2">
                        ...
                        <div class="flex items-center text-xs ml-2">
                            ...
                            <span
                                class="{{ $connectionStatus === 'connected' ? 'text-green-600' : ($connectionStatus === 'fallback' ? 'text-yellow-600' : 'text-red-600') }}">
                    ...
                    @if ($status === 'break')
                        <div
                            class="mt-2 md:mt-0 md:ml-2 px-4 py-2 bg-yellow-50 text-yellow-800 border border-yellow-300 rounded-md text-sm shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="bg-white shadow rounded-lg p-8 space-y-8">
                    ...
                    <div class="relative w-full max-w-sm mx-auto">
                        @if ($status === 'break')
                            <div class="absolute inset-0 z-10 bg-black  flex items-center justify-center rounded-xl">
                        ...
                        @endif
                        <div class="flex flex-col justify-between w-full mx-auto rounded-xl shadow-md border border-kiosqueeing-primary bg-white overflow-hidden"
                        ...
                        <div class="grid grid-cols-4 sm:grid-cols-4 gap-4 mt-8 ">
                            ...
                            <button wire:click="anounceNumber" wire:loading.attr="disabled" @disabled(!$currentTicket)
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <span class="text-xs px-3 py-1 bg-gray-100 rounded-full border text-gray-700">
                                    Left Today:  {{ $queueCountToday }}
                                </span>
                                @if ($forwardedCountToday >0 )
  <span
                                    class="text-xs px-3 py-1 bg-blue-100 rounded-full border border-blue-300 text-blue-700">
                                    Forwarded Today: <strong> {{ $forwardedCountToday }}</strong>
                                </span>
                                @endif
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-2">
                            @forelse ($nextTickets as $next)
                                <button type="button" wire:click="selectQueue({{ $next->id }})"
                                    wire:loading.attr="disabled" @disabled($status === 'break')
                                    ...
                                    <div
                                        class="absolute top-0 rounded-sm right-0 text-[10px] font-bold {{ $status === 'break' ? 'text-gray-500' : 'bg-black text-white px-1.5 py-0.5 rounded-sm' }}">
                                        {{ \Carbon\Carbon::parse($next->created_at)->timezone('Asia/Manila')->format('h:i A') }}
                                    </div>
   <!-- ✅ Forwarded Badge -->
          @if($next->is_forwarded)
    <span
        x-data="{ showTooltip: false }"
        @mouseenter="showTooltip = true"
        @mouseleave="showTooltip = false"
        class="absolute animate-pulse bottom-0 left-0 text-[10px] font-bold bg-white text-denim-700 px-1.5 py-0.5 rounded-tr-md border-t border-r border-denim-400 shadow-sm cursor-help"
    >
        🔁 Forwarded from {{ $next->previousService->name ?? 'Unknown' }}

        <!-- 🧠 Tooltip -->
        <div
            x-show="showTooltip"
            x-transition
            class="absolute bottom-full left-0 mb-1 w-48 bg-gray-800 text-white text-[10px] font-normal rounded-md shadow-lg px-2 py-1 z-50"
            style="white-space: normal;"
        >
            <p><strong>Reason:</strong> {{ $next->forward_reason ?? 'No reason provided' }}</p>
            <p><strong>By:</strong> {{ optional($next->forwardedBy)->name ?? 'Unknown' }}</p>
        </div>
    </span>
@endif
                                    ...
                                    <!-- Big number -->
                                    <div
                                        class="text-4xl sm:text-5xl font-bold {{ $status === 'break' ? 'text-gray-500' : 'text-white' }}">
                                        {{ $next->number }}
                                    </div>

                                    <!-- Ticket number -->
                                    <div
                                        class="text-xs mt-1 {{ $status === 'break' ? 'text-gray-500' : 'text-denim-200 font-bold' }} tracking-wide">
                                        {{ $next->ticket_number }}
                                    </div>

                                    <!-- Service name -->
                                    <div
                                        class="text-xs {{ $status === 'break' ? 'text-gray-500' : 'text-denim-100 font-bold' }} mt-1">
                                        {{ $next->service->name ?? 'No Service' }}
                                    </div>
                                </button>
                            @empty
                                <div
                                    class="col-span-3 flex flex-col items-center justify-center px-6 py-12 bg-gray-100 rounded text-gray-500 border border-gray-200 border-dashed">
                    ...
                    <div>
                        ...
                        @if ($holdTickets->count() > 0)
                            <select wire:model="selectedHoldTicket" wire:change="triggerResumeSelectedHold"
                                ...
                                @foreach ($holdTickets as $hold)
                                    <option value="{{ $hold->id }}">
                                        {{ $hold->number }} - {{ $hold->service->name ?? 'No Service' }}
                                        ({{ $hold->hold_reason ?? '' }})
                                    </option>
                                @endforeach
                            </select>
                        @else
                            <div
                                class="flex flex-col items-center justify-center px-6 py-8 bg-gray-100 rounded text-gray-500 border border-gray-200 border-dashed mb-4">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 mb-2 text-gray-400"
                                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    function enterFallbackMode() {
                        if (!isInFallbackMode) {
                            ...
                            Livewire.dispatch('connectionStatusUpdate', {
                                status: 'fallback'
                            });
                            ...
                            pollingInterval = setInterval(function() {
                                ...
                                Livewire.dispatch('refreshFromEcho', {
                                    fallback: true
                                });
                            }, POLLING_INTERVAL);
                        }
                    ...
                    serviceIds.forEach(function(serviceId) {
                        ...
                        window.Echo.channel(channelName)
                            .listen('.queue.updated', function(event) {
                                console.log('Received queue update for branch ' + branchId + ', service ' +
                                    serviceId + ':', event);
